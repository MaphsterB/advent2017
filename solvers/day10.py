...
0 to 255) and the sequence of lengths in your puzzle input.
...
to bytes using their ASCII codes. This will allow you to handle arbitrary ASCII
...
given 1,2,3, your final sequence of lengths should be 49,44,50,44,51,17,31,73,47,23
...
def hash_round(lengths, cur_pos=0, skip_size=0, modulus=256):
    ...
    for L in lengths:
        ...
        if ei >= si:
            array[si:ei] = reversed(array[si:ei])
        else:
            span = list(reversed(array[si:] + array[:ei]))
            array[si:] = span[:modulus - si]
            array[:ei] = span[modulus - si:]
        cur_pos = (cur_pos + L + skip_size) % modulus
        skip_size += 1
...
def part1(input_lines):
    ...
    """
    lengths = [int(x) for x in re.split(r"\s*,\s*", "".join(input_lines))]
    (array, _, _) = hash_round(lengths)
